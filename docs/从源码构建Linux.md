...
可以去内核归档网站([kernel.org](https://kernel.org)) 下载内核源代码。本文固定使用 `linux-7.0.8`，读者也可以选择相近的小版本内核；如果版本跨度较大，少量配置项名称或默认值可能会变化。进入网站后，点击下载你想要版本的 tarball 即可。
...
>
> 如果不用`-nographic`参数，也可以选择让日志输出到QEMU的图形窗口中；但这需要内核同时启用VGA/虚拟终端相关支持。本文后续统一使用串口控制台，所以仍保留`console=ttyS0`。
...
这时，一个最小可用的Linux系统就跑起来了。你可以用`ls`查看文件，用`sh`执行基础的脚本；如果 BusyBox 配置中启用了`vi` applet，也可以用`vi`编辑文本。
...
不得不说写一个支持文件系统访问的BootLoader还是非常复杂的，这里一个文章放不下，我将会在下方提供打包好的源码下载：[bootloader.tar.gz](./assets/bootloader.tar.gz)。后续“从硬盘启动”章节使用的是这个完整源码包中的实现，而不是上面展示的极简保护模式示例。
...
- `ESI`：必须存放指向 `boot_params` 结构体的物理绝对地址。这个启动参数页也常被称为 Zero Page，其中包含一个 `setup_header` 子结构。
...
无论是实模式还是保护模式引导，Bootloader 传递给内核最核心的资产就是 `boot_params` 结构体（在内核源码 `arch/x86/include/uapi/asm/bootparam.h` 中定义）。其中 `setup_header` 是 `boot_params` 中的一段头部字段，bootloader需要读取它提供的内核能力信息，并填入协议要求的启动参数。

Bootloader 需要重点处理这个结构体中的以下信息，内核才能正常初始化；其中有些字段由bootloader填写，有些字段则是内核镜像提供给bootloader读取的能力描述：

| 偏移量/字段 | 变量名         | 含义与约定                                               |
| --------------- | ------------------ | ------------------------------------------------------------ |
| `0x01E8`        | `e820_entries`     | E820 内存物理布局条目的数量                                  |
...
| `0x0234`        | `relocatable_kernel` | 内核是否支持被加载到非默认地址；这是内核提供给bootloader读取的能力字段 |
| `0x023C`        | `hardware_subarch` | 硬件子架构（PC 兼容机填 0）                                  |
| `0x02D0`        | `e820_table`       | 从 BIOS 15h 探测到的物理内存地图表（E820 Map）           |

> [!note]
>
> 为什么叫 "Zero Page"？因为早期的内核实现中，这个结构体被放在实模式内存的 `0x90000` 处（初始化为全0，然后填入参数），故而得名。

通常，x86下的32位BootLoader会将内核镜像加载到1MB处，启动参数则加载到`0x90000`（576KB）处。现代Linux内核通常支持重定位，但这不是bootloader单方面修改`relocatable_kernel`就能做到的：`relocatable_kernel`是内核写在setup header里的能力声明，bootloader应先读取该字段，确认支持后再选择满足协议和对齐要求的加载地址，并填好相应启动参数。
...
经过刚才的一番折腾，我们已经可以彻底抛弃`initramfs`了。这里需要先解压前文提供的完整BootLoader源码包，并在该源码目录中执行安装脚本，将BootLoader写入已经准备好的磁盘镜像，然后直接从磁盘启动。
...
BIOS是计算机发展的早期遗留产物，如上所见，其具有诸多缺点。传统BIOS提供给启动程序的主要接口工作在16位实模式下，编写阅读困难，连续寻址空间有限；很多固件服务通过中断调用暴露，调用方式需要查询手册，性能和表达能力都不适合复杂的现代启动流程。然而，由于其肩负众多计算机硬件及操作系统的历史债务，常年来也很难将其彻底更换。

但说到底，BIOS也不过是一种特殊的软件，只不过这个软件的代码被固化到了主板芯片上，而非存储在硬盘里。随着芯片技术的发展，固件存储容量也逐渐不再是限制，各方都需要一种更先进、更友好且更易于扩展的启动协议。1998年左右，芯片巨头Intel开始着手设计EFI，并在几年后成立UEFI Forum，将其转为行业标准。

到2010年左右，日渐成熟的UEFI开始逐渐占领主流x86主板市场，微软也在Windows8开始推广使用UEFI，而2010年代后期，UEFI彻底成为新设备的标准配置。

相较于BIOS，UEFI提供了更现代的固件接口和C风格的编程模型。它在完成早期初始化后通常会进入32位或64位环境，并可以支持大量以前不得不放弃的功能，包括图形、鼠标、网络等更多设备驱动，更多文件系统的驱动和更好的操作系统融合。有了更强大的功能支撑，UEFI不再局限于“BIOS读取一个扇区再交给BootLoader”的方式，而是可以从文件系统加载EFI应用、boot manager，甚至通过Linux EFI stub更直接地加载内核。如今我们看到的带有鼠标的“BIOS”，可以和开机屏幕融合的操作系统启动界面，很多都建立在UEFI提供的能力之上。

> [!tip]
>
> 当然对于x86架构的CPU而言，UEFI在上电之初也不得不运行在16位实模式下，但和只能运行在实模式下的BIOS不同，UEFI会尽快完成初始化，让自己进入32位或64位的现代模式下。

## 结语

写这篇文章确实消耗不少时间，也做了很多测试，文章中的所有命令和代码均在我的WSL2 Debian环境中实际测试过，大多数代码读者可以复制粘贴，我尽可能避免了交互过程。其实还有一些想写的东西，比如现代发行版中常见的从`initramfs`进行`switch_root`，但我觉得这里单开一章可能会更好一些，以便可以单独了解Linux常见的启动过程，刚巧Linux7.0发布了`nullfs`这个新根文件系统，想必此后发行版的init程序可以直接使用`pivot_root`让事情变得更简单。这次的文章比起以前长了很多，算是一种新的尝试，之后再想想应该写什么。
