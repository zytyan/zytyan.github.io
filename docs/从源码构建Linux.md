...
sudo apt install make gcc \
	flex bison bc libelf-dev lbzip2 bzip2 pkg-config \
	libncurses-dev -y
```
...
```bash
# Debian自带的gawk可能有些老了
sudo apt update && sudo apt upgrade -y gawk
# 创建构建目录，构建结果将会放在这里
mkdir build && cd $_
...
这次编译将会经在stage目录下产生了大量文件：包括标准的C库、数学计算库、加载器和头文件。这将是用户态Linux世界的基础支柱。
...
```bash
mkdir ../glibc-2.43/build/stage/bin/ && cp busybox $_
...
安装busybox后，就可以使用它的applet进行Linux基础的操作了。此时还无法直接使用普通的Linux命令，需要使用`/bin/busybox --install -s`安装，这会自动创建buxybox的软链接，此后busybox就可以根据启动的是哪个软链接来决定自己要做什么。

> [!important]
>
> 使用`/bin/busybox --install`时，必须要用绝对路劲，否则busybox会拒绝安装。这是因为busybox不能假设它能获取自身的绝对路径，文章后面的部分会解释为什么会有这种情况。
...
```bash
find . -print0 | cpio -H newc -0 -o > ../../../minilinux.cpio
cd ~/linux-7.0.8
...
事实上这是因为qemu默认分配的内存只有128MB，而我们生成的cpio有足足109M，加上内核占用后几乎没有剩余内存，内核在解包cpio时由于内存不足触发CPU重置，导致虚拟机整机重启。解决方案很简单，增加qemu的内存即可。使用`-m 1G`将内存增加到1G。

> [!note]
>
> 具体路径是这样的：首次内存不足时，触发了缺页异常(#GP)，然而由于内存不足，投递#GP的时候又触发了#GP，这导致CPU不得不将其合并为一个double fault(#DF)，然而投递这个#DF的时候还是内存不足，又触发了#GP，触发了triple fault，而CPU无法处理triple fault，所以不得不reset，也就是看到了反复重启。通常来讲，内核会为DF专门准备一个单独的执行栈，尽可能避免缺页在异常处理时发生，然而这次异常发生在极早期，内核还没来得及准备。

使用下面的命令重启启动qemu，此时就可以进入系统了。
...
重新启动后，你会看到屏幕循环打印`can't optn /dev/tty2`这样的信息，这是因为我们的`/dev`目录下还没有自动生成设备节点。在WSL2中可以用`ls /dev`查看Linux内核自动生成的设备节点。这就需要我们重新配置内核，打开`devtmpfs`选项。记得也要勾选`Automount devtmpfs at /dev, after the kernel mounted the rootfs`。而更下面的`Use nosuid,noexec mount options on devtmpfs`是安全选项，目前我们的系统开它与否没有区别，可以根据心情开。
